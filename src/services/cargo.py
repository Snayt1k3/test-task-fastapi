...
from src.dto.cargo import CargoSchemaAdd, CargoSchemaEdit, CargoSchema, CargoSchemaFilter
...
class CargosService:
    async def add_cargo(self, uow: IUnitOfWork, cargo: CargoSchemaAdd) -> int:
        ...
        async with uow:
            ...
            cargo_dict.update({"delivery_id": delivery_location.id, "pick_up_id": pick_location.id})
            cargo_id = await uow.cargos.add_one(cargo_dict)
            await uow.commit()
            return cargo_id

    async def get_cargos_with_machines(self, uow: IUnitOfWork, filter: CargoSchemaFilter):
        """Получение всех грузов и ближайших машин к ним"""

        r_dict = {"cargos": []}

        async with uow:
            all_cargos = [cargo for cargo in await uow.cargos.find_all() if filter.weight is None or cargo.weight == filter.weight]
                ...
                r_dict["cargos"].append(
                    {
                        ...
                        "nearest_cars": len([machine for machine in all_machines if geodesic((machine.location.latitude, machine.location.longitude),
                                                                (cargo.pick_up_location.latitude,
                                                                 cargo.delivery_location.longitude)).miles <=filter.miles])
                    }
                )
        return r_dict
    ...
    async def get_cargo_and_machines(self, uow: IUnitOfWork, id: int):
        ...
        async with uow:
            ...
            for machine in all_machines:
                r_dict["machines"].append({"car_number": machine.identifier,
                                           "distance": geodesic((machine.location.latitude, machine.location.longitude),
                                                                (cargo.pick_up_location.latitude,
                                                                 cargo.delivery_location.longitude)).miles
                                           })
    ...
    async def edit_cargo(self, uow: IUnitOfWork, cargo: CargoSchemaEdit):
        """Обновляет груз вбд"""
        async with uow:
            id = await uow.cargos.edit_one(cargo.id, {"description": cargo.description, "weight": cargo.weight})
            await uow.commit()
            return id
